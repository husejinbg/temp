...
public:
	...
	// Getters are const:
	int getX() const { return x; }         // Accessor for x coordinate
	int getY() const { return y; }         // Accessor for y coordinate
...
private:
	...
	const int MIN_y{};
	int x{ MIN_x }, y{ MIN_y };	  // x ve y coordinates are initialized 
	mutable unsigned int printCount{};  // A mutable variable to count how many times a point object is printed
};
...
void Point::print() const
{
	...
	std::cout << "Print count= " << ++printCount << std::endl;  // increment and print the mutable printCount
}

// Setters
void Point::setX(int new_x) {
	if (new_x >= MIN_x) x = new_x;  // Accepts only valid values 
}
void Point::setY(int new_y) {
	if (new_y >= MIN_y) y = new_y;  // Accepts only valid values 
}
