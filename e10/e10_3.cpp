...
int main()
{
	std::weak_ptr<ColoredPoint> smart_ptr1;

	{	// A new scope
		std::shared_ptr<ColoredPoint> smart_ptr2 {new ColoredPoint{ 10, 20, Color::Green }};  // using new operator
		smart_ptr1 = smart_ptr2;		// weak_ptr points to shared_ptr
		cout << smart_ptr1.use_count() << endl;	// Number of pointers sharing the same object. weak_ptr does not count
		cout << "End of scope" << endl;
	} // End of scope. The object will be deleted.

	cout << "After the scope" << endl;
	// smart_ptr1->print();  The object does not exist
	// smart_ptr still exist.
	cout << smart_ptr1.use_count() << endl;	// Number of pointers sharing the same object. weak_ptr does not count
